...
**Treat the exported bundle as a live credential, because it is one — full account access, not a scoped token.** **Save it (or move it) straight into `.qa/`, the SAME directory `--save-session` already writes to and `.gitignore` already excludes** (`.qa/`, verified in this repo's own `.gitignore` — not a claim about tooling outside it) — never at the working directory's top level or anywhere else, where nothing protects it from an accidental `git add`. Delete it once you've used it; never let it land in a report, a screenshot, or anything committed or shared.

Two ways to produce a session bundle in exactly the shape `--session`/`--save-session` already read and write (`{"user_agent": "...", "storage_state": {"cookies": [...], "origins": [{"origin": "...", "localStorage": [...]}]}}`), saved as `.qa/session.json` either way:

- **By hand — works regardless of how you obtained this skill.** In the browser you're already logged into: DevTools → Application → Cookies (copy each cookie's `name`/`value`/`domain`/`path`/`secure`/`httpOnly`/`sameSite`) and Local Storage (copy each `name`/`value`), and `navigator.userAgent` from the Console. Assemble them into the same JSON shape by hand and save the file as `.qa/session.json` directly — create the `.qa/` directory first if this is a fresh checkout.
- **The extension**, if you're working in the `qa-tool` development workspace this skill ships from (not part of the distributed plugin itself — a marketplace install of just this skill won't have it): `browser-extension/` at that workspace's root, in-house use only, loaded via the browser's own "load unpacked extension" developer mode. Open the target site in a tab where you're already logged in, click the extension icon, click **Export Session** — it reads that tab's cookies (via the browser's own cookie API, so `HttpOnly` cookies are included, unlike `document.cookie`) and localStorage, and downloads a bundle to your Downloads folder (outside this repo's `.gitignore` coverage) — **move it into `.qa/` immediately**, then delete the Downloads copy.
