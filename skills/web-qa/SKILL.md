...
**Treat the exported bundle as a live credential, because it is one — full account access, not a scoped token.** Delete it once you've used it; never let it land in a report, a screenshot, an artifact, or anything committed or shared. It carries exactly the same sensitivity as `.qa/session.json` already does in this skill (gitignored, and `sync_skills.py` refuses to sync anything credential-shaped) — a downloaded file just makes it easier to forget about.

Two ways to produce a `storage_state.json` in exactly the shape `--session`/`--save-session` already read and write (`{"user_agent": "...", "storage_state": {"cookies": [...], "origins": [{"origin": "...", "localStorage": [...]}]}}`):
