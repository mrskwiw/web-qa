...
**Treat the exported bundle as a live credential, because it is one — full account access, not a scoped token.** `.qa/` is this skill's own convention for session storage (`--save-session` already writes there), and it's excluded in every repo *this* project ships from — but that guarantee is about *this* project, not about whatever project you're running the skill against. **Before trusting `.qa/` in that project, verify it properly** — a single `git check-ignore -v .qa/session.json` match is not enough by itself: `git check-ignore -v .qa/session.json`. Confirm the reported source is that repo's OWN committed `.gitignore` — a match against a personal global exclude or the local, unshared `.git/info/exclude` protects nothing on any other checkout or in CI. Separately confirm the path isn't already tracked: `git ls-files .qa/` must return nothing (an ignore rule never retroactively untracks a file git already knows about). If either check fails, add an explicit rule to that repo's own `.gitignore` (and `git rm --cached` anything already tracked) before saving into it, or save outside any git working tree entirely. Never assume; a live credential landing in a commit because the assumption held everywhere else it's been used is exactly the failure mode to design out. Delete the bundle once you've used it either way; never let it land in a report, a screenshot, or anything committed or shared.
