...
**Pass `--output` on `act`/`flow` whenever you'll judge the produced content afterward.** `content_after` can run to 20,000 chars per step (BUGS.md 2026-08-21) — on a 15-action flow that alone can approach your whole context budget. Passing `--output <file>` makes stdout show only a ~500-char excerpt (with a `[stdout truncated: showing N of M chars — full content in <file>]` marker); the file always holds the untruncated bundle. Omit `--output` and stdout is full-length exactly as before — nothing is ever silently lost, but you pay the full cost every time. Read the file when the excerpt isn't enough to judge the outcome (e.g. a scorecard or list that renders past the excerpt).
...
- Spawn subagents with the **Agent tool, all in a single message** so they run concurrently. Cap at **~4–6 live at once** (each launches its own headless Chromium — memory-heavy); batch any remainder. That cap assumes a normal, otherwise-idle host — it is not adaptive, and a live incident (2026-09-22, `BUGS.md`) showed a single *non*-fanned-out run get killed by low system memory the host was already under from unrelated processes. If a command has recently been slow, flaky, or killed outright, or you have any other reason to think the host is tight on memory, shrink the batch (2–3, or strictly serial) rather than assuming 4–6 is safe by default, and pass every subagent `--low-memory` (a real engine flag as of 2026-09-22 — conservative Chromium args that reduce each instance's baseline footprint at no functional cost). Treat a subagent that never returns as a signal to check for exactly this, not just a timeout to retry blindly.
...
- **The final report** — merging fragments, deduping issues raised by more than one subagent, assigning final `issue-NNN` ids, and rendering is one serial `report` pass by you (§4). **Dedup only true duplicates** — the *same defect* two subagents both hit (same `category` + same target element/URL + materially the same evidence), never merely two findings that share a page URL. Multiple distinct bugs on one page are the norm; collapsing on title+url alone would silently drop real findings.
...
This solves a narrower problem than it might sound like: getting past a login flow that **cannot be scripted at all** (OAuth/SSO, MFA, passkeys) without driving it end-to-end a second time in `record`. **It does not defeat bot-detection that fingerprints the automation tool itself** — replaying a borrowed session still runs through this engine's own Playwright-controlled Chromium, which carries its own signals (`navigator.webdriver`, CDP artifacts, TLS/JA3 fingerprint) independent of whether the cookies are valid. A site that blocks *automation*, not merely unauthenticated requests, can still block a replay carrying a perfectly legitimate borrowed session. That is the same wall `login`/`record` already can't get through automated, and this project's stance on it is unchanged: don't fight it (§ Authentication in `web-drive`'s `SKILL.md` states this plainly, and it applies here too).

**Treat the exported bundle as a live credential, because it is one — full account access, not a scoped token.** `.qa/` is this skill's own convention for session storage (`--save-session` already writes there), and it's excluded in every repo *this* project ships from — but that guarantee is about *this* project, not about whatever project you're running the skill against. **Before trusting `.qa/` in that project, verify it properly, with two checks:** (1) the rule is actually **committed**, not just sitting in an uncommitted edit that protects nobody else's checkout or CI — `git show HEAD:.gitignore` must contain a line covering `.qa/`; (2) the path isn't **already tracked** — `git ls-files .qa/` must return nothing, since an ignore rule never retroactively untracks a file git already knows about. If either check fails, add and commit an explicit rule to that repo's own `.gitignore` (and `git rm --cached` anything already tracked) before saving into it, or save outside any git working tree entirely. Never assume; a live credential landing in a commit because the assumption held everywhere else it's been used is exactly the failure mode to design out. Delete the bundle once you've used it either way; never let it land in a report, a screenshot, or anything committed or shared.

Two ways to produce a session bundle in exactly the shape `--session`/`--save-session` already read and write (`{"user_agent": "...", "storage_state": {"cookies": [...], "origins": [{"origin": "...", "localStorage": [...]}]}}`):

- **By hand — works regardless of how you obtained this skill.** In the browser you're already logged into: DevTools → Application → Cookies (copy each cookie's `name`/`value`/`domain`/`path`/`secure`/`httpOnly`/`sameSite`) and Local Storage (copy each `name`/`value`), and `navigator.userAgent` from the Console. Assemble them into the same JSON shape by hand and save it to the verified-ignored location from above.
- **The extension**, if you're working in the `qa-tool` development workspace this skill ships from (not part of the distributed plugin itself — a marketplace install of just this skill won't have it): `browser-extension/` at that workspace's root, in-house use only, loaded via the browser's own "load unpacked extension" developer mode. Open the target site in a tab where you're already logged in, click the extension icon, click **Export Session** — it reads that tab's cookies (via the browser's own cookie API, so `HttpOnly` cookies are included, unlike `document.cookie`) and localStorage, and downloads a bundle to your Downloads folder — never itself covered by any project's `.gitignore` — so move it to the verified-ignored location before doing anything else with it.

**Either way, capture the browser's real `user_agent`, never assume one — it costs nothing and some sites need it.** Sessions are **commonly** (not universally) bound to the UA+IP fingerprint they were issued under (§ Conventions); on a site that does enforce this, replaying under Playwright's default Chromium UA reads as a fingerprint mismatch, not a bad cookie, and cannot be told apart from an expired token symptomatically — you'll misdiagnose it as needing a fresh login, which is the one thing that cannot help. On a site that doesn't enforce it, a replay failure means something else broke; don't assume UA mismatch as the default explanation without checking.

**Both paths have two real scope limits, not one.** (1) They capture ONE origin — the site you're actually on. If the app keeps auth state on a separate identity/app subdomain, the bundle can be syntactically valid and still missing what that other origin needs. (2) They capture cookies + localStorage only — the same scope `--save-session` has always used (Playwright's own `storage_state`), never sessionStorage, IndexedDB, or service-worker-cached state; a site that keeps auth in one of those produces a bundle that looks complete and isn't. Either gap fails looking exactly like an expired session rather than an incomplete one. **Verify the replay is actually authenticated** (logged-in chrome visible, no redirect back to a login page) before trusting the bundle for anything further — don't just confirm the file loaded without error.
...
1. **Get the output where the user would see it.** Read the evidence bundle's `content_after` (the readable rendered text) — pass `--output` on the triggering `act`/`flow` call (§ Setup) if the excerpt on stdout isn't enough to judge this outcome, and read the file for the full text. If the result lives on another view (a results tab, a detail page, a downloaded file), **drive there and read it** — navigate to the results/detail route, open the record, or fetch the export. The output is often not on the page where you triggered it.
...
For the deterministic layer, a `flow` step can assert `content_contains` / `content_absent` (e.g. result panel shows the entity, and does **not** say "No results"/"Error") — but those are only a coarse net. **Whether the produced content is genuinely good is your semantic judgment, and it is not optional for output-producing actions.** A separate assertion, `dom_contains`, matches against the `dom_outline` role/text tree of *interactive + landmark* nodes, not rendered body text — a page whose result is plain text (a confirmation, an error, a bare heading) has an EMPTY outline and every `dom_contains` against it fails even though the text is right there. Use `content_contains` for a text outcome; reserve `dom_contains` for asserting a control or landmark exists.
...
Assemble a results object and render it. **When you fanned out (Orchestration §), this is the serial merge step:** union the subagents' fragments into one `issues[]`, dedup only true duplicates (the *same defect* two subagents both hit — same `category` + same target element/URL + materially the same evidence — never merely two findings sharing a page URL, since distinct bugs commonly share one), renumber to contiguous `issue-NNN`, and render **once** — subagents never render their own report. **Save every report under the repository-root `reports/` directory** (create it if missing), in a per-run folder named `<site-slug>-<YYYY-MM-DD>`:
...
**Safe by default.** The sweep probes only read-only (GET) endpoints unless you pass `--include-mutating`; probing a write verb sends a *real* request that would **execute** on an unprotected endpoint (a live `DELETE`/`cache-clear`). Only add `--include-mutating` against a **test** target you own. Doing so also requires `--yes` on the same call — a mutating sweep refuses to run otherwise (`refused: true`, nothing probed), the same self-declared-risk gate as `flow --destructive`/`--costs`: the session's own Bash permission prompt is the outer backstop, but this flag fires real writes at *every* endpoint the spec advertises, and a human skimming a long command for that risk can miss it. The result's `coverage` field says whether the run was complete: **a safe (read-only) sweep with zero findings is NOT a clean bill of health** — if `coverage` is `PARTIAL`, unauthenticated *writes* (the highest-risk bypasses) were never tested, so **report that explicitly** ("read-only sweep: N write endpoints not probed — re-run with `--include-mutating --yes` on a test target"). Never present a partial sweep as "no auth issues." Manual method:
